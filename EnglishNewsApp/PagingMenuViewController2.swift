...
class PagingMenuViewController2: UIViewController,UITableViewDelegate,UITableViewDataSource,XMLParserDelegate{
    
    @IBOutlet weak var tableView: UITableView!
    
    let feedUrl = URL(string: "http://feeds.bbci.co.uk/news/world/asia/rss.xml")!
    var feedItems = [DeedItem]()
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String] = [:]) {
        ...
        if elementName == ITEM_ELEMENT_NAME {
            self.feedItems.append(DeedItem())
        } else {
